...
void main() {
  ...
  testWidgets(
    'teacher shortcuts are replaced with full names from the directory',
    (tester) async {
      final repository = fixtureRepository(
        sessionManager,
        'week_with_double_hours.html',
  ...
  testWidgets('shows the expired-session state with a login button', (
    tester,
  ) async {
    final repository = StundenplanRepository(
      sessionManager: sessionManager,
  ...
  testWidgets('live double hour gets the live frame, no JETZT badge', (
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 8, 30)),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('single-hour lessons track their own 45-minute window', (
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 10, 30)),
    );
    for (var i = 0; i < 6; i++) {
  ...
  testWidgets('no live badge outside school hours', (tester) async {
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 15, 30)),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('no now line before the school day starts', (tester) async {
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 7, 30)),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('now line appears exactly at the start of the first lesson', (
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 8, 10)),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'a day without afternoon shows no Mittagspause and ends at lunch',
    (tester) async {
      final repository = fixtureRepository(
        sessionManager,
  ...
  testWidgets('few blocks stretch evenly to fill the screen height', (
    tester,
  ) async {
    final repository = fixtureRepository(
      sessionManager,
    ...
    // Nur 3 Blöcke (1+2, 3+4, 5+6) auf dem Test-Screen: Die Karten werden
    // deutlich über ihre natürliche Höhe gestreckt ...
    final cardHeights = [
      for (final card in tester.widgetList<ClassCard>(find.byType(ClassCard)))
        tester.getSize(find.byWidget(card)).height,
    ];
    expect(cardHeights, isNotEmpty);
    for (final h in cardHeights) {
      expect(h, greaterThan(100));
    }

    // ... und füllen den Screen bis fast ganz unten (kein leerer Raum).
    final lastBottom = tester.getBottomLeft(find.byType(ClassCard).last).dy;
    expect(lastBottom, greaterThan(480));

    // Gleichmäßige Aufteilung: alle Zeilen exakt gleich hoch.
    for (final h in cardHeights) {
      expect(h, closeTo(cardHeights.first, 0.5));
    }
  });

  testWidgets(
    'now line hides during the Mittagspause, countdown pill appears',
    (tester) async {
      ...
      await tester.pumpWidget(
        buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 12, 50)),
      );
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('now-line')), findsOneWidget);
      ...
      await tester.pumpWidget(
        buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 13, 25)),
      );
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('now-line')), findsNothing);
      ...
      await tester.pumpWidget(
        buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 13, 40)),
      );
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('now-line')), findsNothing);
      ...
      await tester.pumpWidget(
        buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 14, 30)),
      );
      await tester.pumpAndSettle();
      expect(find.byKey(const Key('now-line')), findsOneWidget);
      expect(find.textContaining('Endet in'), findsNothing);
  ...
  testWidgets('now line moves proportionally to the elapsed minutes', (
    ...
    expect((atQuarter - atStart) * 3, closeTo(atHalf - atStart, 2));
  });
  ...
  testWidgets(
    'left column times have a solid background to mask the now line',
    (tester) async {
      final repository = fixtureRepository(
        sessionManager,
        'week_with_double_hours.html',
  ...
  testWidgets('now line time pill sits on the right side', (tester) async {
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 9, 0)),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'swiping right goes from Friday to Thursday, left back to Friday',
    (tester) async {
      final repository = fixtureRepository(
        sessionManager,
        'week_with_double_hours.html',
  ...
  testWidgets('upcoming classes get an urgency badge with countdown', (
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 8, 6)),
    );
    for (var i = 0; i < 6; i++) {
      await tester.pump(const Duration(milliseconds: 50));
    }
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 8, 9)),
    );
    for (var i = 0; i < 6; i++) {
      await tester.pump(const Duration(milliseconds: 50));
    }
  ...
  testWidgets('urgency applies only in the 5 minutes before a class', (
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 8, 5)),
    );
    for (var i = 0; i < 6; i++) {
      await tester.pump(const Duration(milliseconds: 50));
    }
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 8, 4)),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('split classes both get the urgency state', (tester) async {
    ...
    await tester.pumpWidget(
      buildTestWidget(repository, clock: () => DateTime(2026, 8, 14, 11, 40)),
    );
    for (var i = 0; i < 6; i++) {
      await tester.pump(const Duration(milliseconds: 50));
    }
